...
Domain: `pddl/numeric/domain_numeric.pddl`, battery modelled as a numeric fluent (0-100). Cost model: navigate=1, shelve=2, recharge=3. Metric: minimize total-cost.
...
- Satisficing search scales way better than optimal search. On problem 3, sat-hadd and sat-hff both return a plan in about 0.25s, while opt-hmax and opt-hrmax don't finish within 120s, which is the exponential blow-up of insisting on optimality showing up in practice.
...
Basically the two models are answering different questions: STRIPS finds the shortest plan, numeric finds the most energy-efficient one. Both are correct answers to what they're each asking, which is kind of the point: the cost model you pick changes what "best plan" even means.
